...
export interface SlideDef { id: string; num: string; title: string; node: ReactNode; tone?: 'dark' | 'light' }
export interface DeckDef {
  ...
  times?: string[]
  /** Duración objetivo de la charla en segundos (default 180 = 3 min). */
  targetSeconds?: number
}

const fmt = (s: number) => `${Math.floor(s / 60)}:${String(Math.abs(s) % 60).padStart(2, '0')}`
 ...
 *  01,02,03…; las demás (portada '', divisor '', preview '▶'…) se conservan. */
...
export default function Deck({ deck, initialSlide = 1 }: { deck: DeckDef; initialSlide?: number }) {
  ...
  const target = deck.targetSeconds ?? 180
  ...
  return (
    <div className="deck" data-overview={overview ? 'on' : undefined} data-theme={cs.tone}>
      ...
      <header className="deck-bar">
        <button className="deck-brand" onClick={() => go(0)} aria-label="AECODE × Notion · ir a la portada">
